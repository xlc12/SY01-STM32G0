...
uint8_t QMC5883_Init()  //初始化模块
{
        uint8_t uChipID = 0;    
        // 设备复位，检查设备
//        QMC_IIC_WriteByte(QMC5883_DEV,0x58,0X01);               //复位
//        QMC_IIC_WriteByte(QMC5883_DEV,QMC5883L_REG_CONF1,CONF1);    //配置控制器1
...
// 全局变量：原始数据、滤波结果、方位角（新增Azimuth）
extern int16_t qmcdata[3];  // 原始三轴数据（X/Y/Z，int16_t类型）
float MagX, MagY, MagZ, MagH;  // 滤波后的数据
float Azimuth = 0.0f;         // 新增：方位角（单位：度，0°=磁北，顺时针递增）
