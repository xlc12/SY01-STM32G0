...
//SCL和SDA都是  选择推挽输出才行,即Output push pull  初始化为低电平LOW, 上拉模式Pull_Up,中速模式Medium(一定要选择,不然OLED花屏)

/*引脚配置*/
#define OLED_W_SCL(x)        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_3, (GPIO_PinState)(x))
#define OLED_W_SDA(x)        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, (GPIO_PinState)(x))
...
void OLED_ShowCN(uint8_t Line, uint8_t Column, uint8_t Num)  //显示汉字
{
  uint8_t i;
  uint8_t wide = 20; //字宽
...
//	OLED_ShowNum(2, 14, KEY_GetMultiClickCount(), 2); //显示按键值次数
    ...
    OLED_ShowFloat(4, 1, Target_Azimuth, 5, 2); //读取南北角度
    OLED_ShowFloat(4, 9, GetTurntableAngle(), 5, 2); //读取马儿角度
}